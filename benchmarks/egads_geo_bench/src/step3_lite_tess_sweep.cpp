...
int main(int argc, char** argv) {
  ...
  std::printf("[TOPO] bodies=%d faces=%d\n", nchild, nfaces);

  // ---- Tessellation sweep ----
  // params = [maxEdgeLen, chordalDev(deflection), dihedralDeg]
  ...
  // IMPORTANT: Skip EG_free(...) here to avoid the double-free crash in this build.
  EG_close(ctx);
  return 0;
}
